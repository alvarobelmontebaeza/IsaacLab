...
class CommandsCfg:
    ...
    ee_pose = mdp.UniformSphericalPoseCommandCfg(
        asset_name="robot",
        body_name=".*link_grasping_frame", # Virtual EE frame at the end of the robot arm
        resampling_time_range=(4.0, 4.0),
        sphere_ranges=mdp.UniformSphericalPoseCommandCfg.SphereRanges(
            radius=(0.2, 0.5),
            polar=(-2.0 * math.pi / 5.0, 2.0 * math.pi / 5.0),
            azimuth=(-3.0 * math.pi / 5.0, 3.0 * math.pi / 5.0),
            roll=(0.0, 0.0),
            pitch=(0.0, 0.0),
            yaw=(0.0, 0.0),
        ),
...
class ActionsCfg:
    ...
    leg_joint_pos = mdp.JointPositionActionCfg(asset_name="robot", joint_names=[".*hip_joint", ".*_thigh_joint", ".*calf_joint"], scale=0.45, use_default_offset=True)
...
class RewardsCfg:
    ...
    '''
    pose_tracking = RewTerm(
        mdp.pose_command_error_spherical,
        weight=10.0,
        params={"command_name": "ee_pose", "asset_cfg": SceneEntityCfg("robot", body_names=[".*link_grasping_frame"])}
    )
    ...
    legs_dof_power = RewTerm(func=mdp.joint_power_l2, weight=-6e-5, params={"asset_cfg": SceneEntityCfg("robot", joint_names=["F.*", "R.*"])}) 
    ...
    # -- constraints
    root_height = RewTerm(func=mdp.root_height_below_minimum, weight=-1.0, params={"minimum_height": 0.28})
    ...
    # base_orientation = RewTerm(func=mdp.flat_orientation_l2, weight=-5.0, params={"asset_cfg": SceneEntityCfg("robot", body_names="trunk")})
