...
class UniformSphericalPoseCommand(CommandTerm):
    ...
    def __init__(self, cfg: UniformSphericalPoseCommandCfg, env: ManagerBasedEnv):
        ...
        self.pose_command_spherical = torch.zeros(self.num_envs, 3, device=self.device)
        self.pose_command_cartesian = torch.zeros(self.num_envs, 7, device=self.device)
        self.pose_command_cartesian[:, 3] = 1.0
        self.pose_command_w = torch.zeros_like(self.pose_command_cartesian)
        # -- metrics
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        ...
        self.pose_command_spherical[env_ids, 0] = r.uniform_(*self.cfg.ranges.radius)
        self.pose_command_spherical[env_ids, 1] = r.uniform_(*self.cfg.ranges.pitch)
        self.pose_command_spherical[env_ids, 2] = r.uniform_(*self.cfg.ranges.yaw)
        # -- orientation
        euler_angles = torch.zeros_like(self.pose_command_cartesian[env_ids, :3])
        euler_angles[:, 0].uniform_(*self.cfg.ranges.roll)
        euler_angles[:, 1].uniform_(*self.cfg.ranges.pitch)
        euler_angles[:, 2].uniform_(*self.cfg.ranges.yaw)
        quat = quat_from_euler_xyz(euler_angles[:, 0], euler_angles[:, 1], euler_angles[:, 2])
        # make sure the quaternion has real part as positive
        self.pose_command_cartesian[env_ids, 3:] = quat_unique(quat) if self.cfg.make_quat_unique else quat
