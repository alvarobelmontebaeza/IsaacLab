...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        joint_pos = ObsTerm(func=mdp.joint_pos, params={"asset_cfg": SceneEntityCfg("robot", joint_names=[".*hip_joint", ".*thigh_joint", ".*calf_joint"])}, noise=Unoise(n_min=-0.01, n_max=0.01))
        joint_vel = ObsTerm(func=mdp.joint_vel, params={"asset_cfg": SceneEntityCfg("robot", joint_names=[".*hip_joint", ".*thigh_joint", ".*calf_joint"])}, noise=Unoise(n_min=-1.5, n_max=1.5))
...
class RewardsCfg:
    ...
    pose_tracking = RewTerm(
        ...
        weight=5.0,
        params={"command_name": "ee_pose", "asset_cfg": SceneEntityCfg("robot", body_names=[".*trunk"])}
    )
