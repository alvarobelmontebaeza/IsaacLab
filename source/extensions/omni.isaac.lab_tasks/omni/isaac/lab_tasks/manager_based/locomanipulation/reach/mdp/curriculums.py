...
def terrain_levels_pos(
    env: ManagerBasedRLEnv, env_ids: Sequence[int], asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"), threshold: float = 0.1
) -> torch.Tensor:
    ...
    # compute the distance the robot walked
    distance = torch.norm(asset.data.body_pos_w[env_ids, asset_cfg.body_ids[0], :] - command[env_ids, :3], dim=1)
    # robots that walked far enough progress to harder terrains
    move_up = distance < threshold
    # robots that walked less than half of their required distance go to simpler terrains
    move_down = distance > threshold * 3.0
    move_down *= ~move_up
    # update terrain levels
    terrain.update_env_origins(env_ids, move_up, move_down)
    # return the mean terrain level
